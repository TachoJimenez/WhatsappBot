...
<div>
	...
	<a class="action-button pull-right"
   	  data-placement="bottom"
  	  data-toggle="tooltip"
  	  title="Copiar reporte para Mantis"
  	  href="#"
  	  onclick="copyMantisReport(); return false;">
   	  <i class="icon-copy"></i>
	</a>
            <?php
            // Assign
...
function copyMantisReport(){
    var ticketId = "<?php echo $ticket->getNumber(); ?>";
    var subject = <?php echo json_encode($ticket->getSubject()); ?>;
    var name = <?php echo json_encode($ticket->getName()); ?>;
    var email = <?php echo json_encode($ticket->getEmail()); ?>;

    // Contenedor del hilo (en osTicket suele ser ticketThread)
    var thread = document.querySelector('#ticketThread') || document.querySelector('#ticket_thread');
    var finalText = subject; // Fallback al asusnto si no hay hilo
    
    if (thread) {
        var entry = thread.querySelector('.thread-entry.message') || thread.querySelector('.thread-entry');
        if (entry) {
            var body = entry.querySelector('.thread-body') || entry.querySelector('.message') || entry;
            var text = (body.innerText || '').trim();
            var lines = text.split('\n').map(function(l){ return l.trim(); }).filter(function(l){ return l.length > 0; });
            lines = lines.filter(function(l){
                if (/posted\s+\d{1,2}\/\d{1,2}\/\d{2,4}/i.test(l)) return false;
                if (/^created by/i.test(l)) return false;
                if (/^whatsapp\s*:/i.test(l)) return false;
                if (/\bbytes\b/i.test(l)) return false;
                return true;
            });
            if (lines.join('\n').trim()) finalText = lines.join('\n').trim();
        }
    }

    if (!confirm('¿Deseas exportar el Ticket #' + ticketId + ' hacia MantisBT?')) return;

    var btn = event.currentTarget || document.activeElement;
    if(btn) btn.innerHTML = '<i class="icon-spinner icon-spin"></i>';
    ...
    fetch('http://127.0.0.1:8083/api/export-to-mantis', {
        method: 'POST',
    ...
    .then(function(data){
        if(btn) btn.innerHTML = '<i class="icon-check"></i>';
        if(data.ok) alert('¡Éxito! Ticket #' + ticketId + ' fue clonado en MantisBT como Issue #' + data.mantisId);
        else alert('Error del Bot: ' + data.message);
    })
    .catch(function(err){
        if(btn) btn.innerHTML = '<i class="icon-warning-sign"></i>';
        alert('Error conectando con el Bot local (Puerto 8083): ' + err);
    });
}
</script>
